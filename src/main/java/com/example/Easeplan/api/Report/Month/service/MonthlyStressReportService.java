...
public class MonthlyStressReportService {
    ...
    public MonthlyStressReportResponse getMonthlyStressReport(String email, YearMonth ym) throws Exception {
        ...
        // 1) 월 범위의 HeartRate 불러와서 일자별 평균(stressEma) 계산
        Map<LocalDate, Double> dailyAvg = new LinkedHashMap<>();
        for (LocalDate cur = first; !cur.isAfter(last); cur = cur.plusDays(1)) {
            long dayStartMs = cur.atStartOfDay(KST).toInstant().toEpochMilli();
            long dayEndMsEx = cur.plusDays(1).atStartOfDay(KST).toInstant().toEpochMilli(); // exclusive

            List<HeartRate> list = smartwatchRepository
                    .findByUserEmailAndTimestampBetween(email, dayStartMs, dayEndMsEx - 1);
        ...
        LocalDate maxDay = dailyAvg.entrySet().stream()
                .max(Comparator.<Map.Entry<LocalDate, Double>>comparingDouble(Map.Entry::getValue)
                        .thenComparing(Map.Entry::getKey))
                .get().getKey();

        LocalDate minDay = dailyAvg.entrySet().stream()
                .min(Comparator.<Map.Entry<LocalDate, Double>>comparingDouble(Map.Entry::getValue)
                        .thenComparing(Map.Entry::getKey))
                .get().getKey();
    ...
    private StressDayReport buildDayReport(User user, LocalDate day, double avg) throws Exception {
        ...
        List<SimpleEvent> emergs = mapByType(events, "emergency");
    ...
    private List<SimpleEvent> mapByType(List<FormattedTimeSlot> events, String type) {
        ...
        return events.stream()
                ...
                .sorted(Comparator.comparing(SimpleEvent::getStartTime))
